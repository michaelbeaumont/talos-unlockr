    ...
    while let Some(attempt) = attempts.next().await {
        ...
        let message = match resp {
            AttemptResponse::Block => {
                ...
                .reply_markup(make_clusters_keyboard(
                    &nodes,
...
pub async fn dispatch(
    ...
    let mut dispatcher = Dispatcher::builder(bot.clone(), handler(user_id))
...
fn make_clusters_keyboard<F>(cluster_nodes: &ClusterNodes, msg: &str, mk: F) -> InlineKeyboardMarkup
where
    F: Fn(String) -> CallbackToggle,
{
    let mut keyboard: Vec<Vec<InlineKeyboardButton>> = vec![];

    for clusters in cluster_nodes.keys().collect::<Vec<_>>().chunks(3) {
        let row = clusters
            .iter()
            .cloned()
            .map(|cluster| {
...
fn handler(user_id: UserId) -> UpdateHandler<anyhow::Error> {
    let command_handler = teloxide::filter_command::<Command, _>().endpoint(unseal);
...
async fn unseal(bot: Bot, nodes: ClusterNodes, dialogue: MyDialogue) -> anyhow::Result<()> {
    log::info!(nodes:?; "Offering clusters to allow action for");
    bot.send_message(dialogue.chat_id(), "Allow one of your clusters:")
        .reply_markup(make_clusters_keyboard(&nodes, "Allow actions", |cluster| {
            ...
            if let Some(message) = query.regular_message() {
                ...
                bot.edit_reply_markup(message)
                    .reply_markup(make_clusters_keyboard(
                        &nodes,
                        "Disallow actions",
                        |cluster| CallbackToggle::Block { cluster },
                    ))
                    .await
